...
struct StatRequest
{
    StatRequest(const std::string& name, const std::string& field) :
        ifName(name), field(field)
    {
    }
    bool operator==(const StatRequest& rhs) const
    {
        return (ifName == rhs.ifName && field == rhs.field);
    }
